...
from agents.autogen_agent import robot_agent, robot_operator, robot_groupchat_manager
import rospy

def handle_execute_tasks(req: ExecuteTasksRequest):
    for task_msg in req.task_list:
        answer = robot_operator.initiate_chat(recipient=robot_groupchat_manager, max_turns=1, message=task_msg.description).summary
